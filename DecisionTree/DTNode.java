...
public class DTNode {
    ...
    public void draw(double x, double y) {
        double left = x - (double) WIDTH / 2;
        String toDraw = text.substring(0, Math.min(text.length(), 15));

        UI.eraseRect(left, y - (double) HEIGHT / 2, WIDTH, HEIGHT); // to clear anything behind it
        UI.drawRect(left, y - (double) HEIGHT / 2, WIDTH, HEIGHT);
